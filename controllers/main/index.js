...
async function contactUsStore(req, res) {
  try {
    ...
    res.render("contact", {
      ...
      errorMessage: {},
      message:
        "Thanks to Share Your Query With Us. Our Team Will Contact Uou Soon",
      data: {},
      session: req.session,
    });
  } catch (error) {
    ...
    mailer.sendMail(
      {
        from: process.env.MAIL_SENDER,
        to: data.email,
        subject: `Your Newsletter Subscription : Team ${process.env.SITE_NAME}`,
        text: `
        Hello Guardian
        Your Newsletter Subscription : Team ${process.env.SITE_NAME}
        Now We Can Mails Regaring New Blogs, Events and Many More...
